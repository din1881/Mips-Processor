...
def Assembler():
    ...
    f = open("test.txt", "r") #r for read , w for write;
    commands = []
...
def write_text():
    #print("I'm in")
    input_value = entry_field.get("1.0", 'end-1c')
    memfile = open("mem.txt", "r")
    regfile = open("reg.txt", "r")

    newfile = open("test.txt", "w+")
    newfile.write(input_value)
    newfile.close()

    Assembler()
    lower_frame_reg.insert(tk.END, regfile.read())
    lower_frame_reg.config(state="disabled")
    lower_frame_mem.insert(tk.END, memfile.read())
    lower_frame_mem.config(state="disabled")
    memfile.close()
    regfile.close()
...
title = tk.Label(text="Please Enter your assembly code: ")
title.grid(column=1, row=0, sticky=NW, padx=200)

entry_field = tk.Text(master=window, height=5, width=30)
entry_field.grid(column=1, row=3, padx=150)


lower_frame_reg = tk.Text(master=window, height=10, width=30)
lower_frame_reg.grid(column=1, row=7, padx=20, pady=20, sticky=E)
lower_frame_reg.config(state= "normal")


lower_frame_mem = tk.Text(master=window, height=10, width=30)
lower_frame_mem.grid(column=1, row=7, pady=20, sticky=W)
lower_frame_reg.config(state= "normal")


button1 = tk.Button(text="Assemble!!", command=lambda: write_text())
button1.grid(column=1, row=5, pady=15)
